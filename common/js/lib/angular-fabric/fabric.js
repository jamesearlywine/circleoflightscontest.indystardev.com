...
.factory('Fabric', [
	...
	return function(options) {
		...
		var self = angular.extend({
			...
			maxContinuousRenderLoops: 0,
			continuousRenderTimeDelay: 5000,
			editable: true,
		...
		self.startContinuousRendering = function() {
			self.continuousRenderCounter = 0;
			// note : removed due to canvas sizing reset issues
			// this fixed a bug (per the service author) for a use-case that we might not have --jle
